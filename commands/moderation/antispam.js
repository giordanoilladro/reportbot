...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('antispam')
    .setDescription('Configura l\'antispam')
  ...
  async execute(interaction) {
    ...
    guildData.antispam.enabled = enable === 'on';

    if (interaction.options.getBoolean('clear_whitelist')) {
      guildData.antispam.whitelistRoles = [];
      guildData.antispam.whitelistUsers = [];
      guildData.antispam.whitelistChannels = [];
    }
    ...
    if (role) guildData.antispam.whitelistRoles.push(role.id);
    if (user) guildData.antispam.whitelistUsers.push(user.id);
    if (channel) guildData.antispam.whitelistChannels.push(channel.id);

    await guildData.save();
    await interaction.editReply({ content: `✅ Antispam ${enable === 'on' ? 'attivato' : 'disattivato'}!\nWhitelist aggiornata.` });
  }
};